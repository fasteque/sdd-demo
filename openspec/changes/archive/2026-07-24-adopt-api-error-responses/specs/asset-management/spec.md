...
- **THEN** the system rejects the request with a 400 status and the standard error envelope (`error.code` `VALIDATION_FAILED`, `error.details` naming the missing field(s)), and does not persist any asset

#### Scenario: Blank tag value
- **WHEN** a client sends `POST /assets` with a `tags` array containing a blank or whitespace-only entry
- **THEN** the system rejects the request with a 400 status and the standard error envelope (`error.code` `VALIDATION_FAILED`, `error.details` naming the `tags` field), and does not persist any asset
...
- **THEN** the system returns a 404 status with the standard error envelope (`error.code` `ASSET_NOT_FOUND`) and does not return an asset body
...
- **THEN** the system returns a 404 status with the standard error envelope (`error.code` `ASSET_NOT_FOUND`) and does not modify any asset data
...
- **THEN** the system rejects the request with a 400 status and the standard error envelope (`error.code` `VALIDATION_FAILED`, `error.details` naming the `page` parameter), and does not return an asset list

#### Scenario: Invalid size value
- **WHEN** a client sends `GET /assets` with a `size` value less than 1
- **THEN** the system rejects the request with a 400 status and the standard error envelope (`error.code` `VALIDATION_FAILED`, `error.details` naming the `size` parameter), and does not return an asset list
